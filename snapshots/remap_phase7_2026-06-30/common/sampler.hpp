...
//
// Phase 7: shape-agnostic core (fetch_impl) — W,H passed explicitly so the SAME
// code serves the fixed-shape TensorView (single W,H) and the var-shape
// VarShapeBatch (per-image imgW[n]/imgH[n]). View must expose value_type,
// channels, and read(int x, int y, int batch, T* out). The thin fetch<T,C>
// wrapper preserves the Phase 1-6 fixed-shape call sites unchanged.
template <typename View>
inline void fetch_impl(const View& v, int x, int y, int z, int W, int H, Border b,
                       const BorderValue& bv, typename View::value_type* out) {
    using T = typename View::value_type;
    constexpr int C = View::channels;
    if (b == Border::Constant && (isOutside(x, W) || isOutside(y, H))) {
        #pragma unroll
        for (int c = 0; c < C; ++c) out[c] = SaturateCast<T>(bv.v[c]);
    } else {
        int bx = (b == Border::Constant) ? x : getIndexWithBorder(b, x, W);
        int by = (b == Border::Constant) ? y : getIndexWithBorder(b, y, H);
        v.read(bx, by, z, out);
    }
}
...
inline void fetch(const TensorView<T, C>& v, int x, int y, int z, Border b,
    ...
    fetch_impl<TensorView<T, C>>(v, x, y, z, v.W, v.H, b, bv, out);
}
...
//
// Phase 6 hot-path: for the vast majority of pixels the ENTIRE interpolation
// neighborhood is in-range, so the border index remap (getIndexWithBorder,
// esp. the integer-modulo REFLECT/WRAP/REFLECT101) is pure waste. We test the
// neighborhood bounds ONCE and, if fully in-range, read directly via v.read()
// (no fetch()/border math). This is BIT-IDENTICAL to the border path because
// getIndexWithBorder is the IDENTITY for in-range indices under ALL border types
// (incl. Constant, which only substitutes borderValue for OUT-of-range indices).
// The interior test is computed identically on host and device -> the CPU ref
// and kernel always pick the same path -> self-consistency (maxerr) is preserved.
//
// Phase 7: shape-agnostic core (sample_impl) — W,H explicit so the same code
// serves TensorView and VarShapeBatch. sample<T,C> wrapper keeps the Phase 1-6
// fixed-shape call sites unchanged.
template <typename View>
inline void sample_impl(const View& v, float fx, float fy, int z, int W, int H,
                        Interp interp, Border b, const BorderValue& bv,
                        typename View::value_type* out) {
    using T = typename View::value_type;
    constexpr int C = View::channels;
    if (interp == Interp::Nearest) {
        ...
        if (ix >= 0 && ix < W && iy >= 0 && iy < H)
            ...
            fetch_impl<View>(v, ix, iy, z, W, H, b, bv, out);
        ...
        if (x1 >= 0 && x2 < W && y1 >= 0 && y2 < H) {
            ...
            fetch_impl<View>(v, x1, y1, z, W, H, b, bv, p00);
            fetch_impl<View>(v, x2, y1, z, W, H, b, bv, p10);
            fetch_impl<View>(v, x1, y2, z, W, H, b, bv, p01);
            fetch_impl<View>(v, x2, y2, z, W, H, b, bv, p11);
        }
        #pragma unroll
        for (int c = 0; c < C; ++c) {
            float acc = 0.f;
        ...
        // 4x4 neighborhood [ix-1,ix+2] x [iy-1,iy+2]; interior iff all in-range.
        ...
        for (int cy = -1; cy <= 2; ++cy) {
            ...
            for (int cx = -1; cx <= 2; ++cx) {
                if (interior)
                    v.read(ix + cx, iy + cy, z, p);
                else
                    fetch_impl<View>(v, ix + cx, iy + cy, z, W, H, b, bv, p);
...
inline void sample(const TensorView<T, C>& v, float fx, float fy, int z,
    ...
    sample_impl<TensorView<T, C>>(v, fx, fy, z, v.W, v.H, interp, b, bv, out);
}

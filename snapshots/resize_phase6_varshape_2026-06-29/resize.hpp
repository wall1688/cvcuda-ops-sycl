...
// ---- LINEAR -----------------------------------------------------------------
// NIX dst pixels per thread with INTERSECT src-read reuse (ported from
// OpResize.cu::LinearInterpolatePack<INTERSECT=true>). Each thread walks NIX
// consecutive dst x; when consecutive dst pixels map to the same src x the 2x2
// neighbourhood is reused, and when src x advances by exactly 1 only 2 of the 4
// pixels are re-read (shift) instead of all 4. This is the big win for upscale
// (scaleX<1 -> many dst pixels share a src pixel). Writes stay scalar (already
// coalesced across the x work-items).
//
// NIX = pack_bytes / pixel_bytes (CUDA DPT/NIX formula): 16-byte pack, or
// 12-byte pack when C==3. Gated to NIX==1 (== simple path, no reuse) for wide
// elements (F32): aligned 4-byte reads are already cheap, and reducing the
// thread count by NIX hurts parallelism more than the saved reads help. The
// reuse pays off for U8, where sub-byte pixels make reads expensive. So for
// sizeof(T)>=4 we keep one dst pixel per thread (simple path); for U8 we use
// the CUDA NIX (16/4/4 for C=1/3/4).
template <typename T, int C>
constexpr int kPackBytes = (C == 3) ? 12 : 16;
template <typename T, int C>
constexpr int kNIX = (sizeof(T) == 1) ? (kPackBytes<T, C> / (int)(sizeof(T) * C)) : 1;
...
inline sycl::event linear_resize(sycl::queue& q, const TensorView<T, C>& src,
    ...
    float scaleX = (float)src.W / (float)dst.W;
    float scaleY = (float)src.H / (float)dst.H;
    constexpr int NIX = kNIX<T, C>;
    size_t gridX = (size_t)((dst.W + NIX - 1) / NIX);

    return q.submit([&](sycl::handler& h) {
        TensorView<T, C> s = src, d = dst;
        h.parallel_for(sycl::range<3>{(size_t)dst.N, (size_t)dst.H, gridX},
            [=](sycl::item<3> it) {
                int z = (int)it[0];
                int y = (int)it[1];
                int bx = (int)it[2];
                int dstCoordX = bx * NIX;
                ...
                // 2x2 neighbourhood (carried across NIX dst pixels).
                ...
                for (int x = 0; x < NIX; ++x) {
                    ...
                    float srcCoordX = (dx + 0.5f) * scaleX - 0.5f;
                    int isx = (int)sycl::floor(srcCoordX);
                    float wx = (isx < 0)          ? 0.f
                             : (isx > src.W - 2)  ? 1.f
                             :                      (srcCoordX - isx);
                    int isx_c = cvsycl::max(0, cvsycl::min(isx, src.W - 2));

                    if (!have || isx_c != iPrevX) {
                        if (have && isx_c == iPrevX + 1) {
                            // src x advanced by 1: reuse 2, read 2 new.
                            #pragma unroll
                            for (int c = 0; c < C; ++c) { p00[c] = p10[c]; p01[c] = p11[c]; }
                            s.read(isx_c + 1, isy,     z, p10);
                            s.read(isx_c + 1, isy + 1, z, p11);
                        } else {
                            s.read(isx_c,     isy,     z, p00);
                            s.read(isx_c + 1, isy,     z, p10);
                            s.read(isx_c,     isy + 1, z, p01);
                            s.read(isx_c + 1, isy + 1, z, p11);
                        }
                        have = true;
                        iPrevX = isx_c;
                    }
                    // (if isx_c == iPrevX, the 2x2 is reused as-is.)

                    float w00 = (1.f - wx) * w00w;
                    float w10 = wx * w00w;
                    float w01 = (1.f - wx) * w1y;
                    float w11 = wx * w1y;

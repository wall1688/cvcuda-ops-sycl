...
// Whole-image dispatch by `code`: picks the right per-family gold and derives
// the family-specific params (sch/dch/bidx/uidx/yidx/FullRange/isSemi) with the
// SAME formulas the correctness test (test_cvt.cpp) uses. Lets the profile
// benchmark time the CPU gold for any code without duplicating the dispatch.
// dst must be sized to the code's out_elems (see test_cvt_profile.cpp sizes());
// src must be sized to the code's in_elems. NHWC flat, single-threaded.
template <typename T>
void cvt_gold_dispatch(T* dst, const T* src, int code, int N, int H, int W) {
  size_t npx = (size_t)N * H * W;

  // Family A: channel swap / alpha add-drop (codes 0-5)
  if (code >= 0 && code <= 5) {
    int sch = (code == 1 || code == 3 || code == 5) ? 4 : 3;
    int dch = (code == 0 || code == 2 || code == 5) ? 4 : 3;
    bool alpha_only = (code == 0 || code == 1);
    cvt_gold::gold_family_a<T>(dst, src, npx, sch, dch, alpha_only);
    return;
  }
  // Family B: BGR->GRAY (codes 6,7,10,11)
  if (code == 6 || code == 7 || code == 10 || code == 11) {
    int bidx = (code == 7 || code == 11) ? 2 : 0;
    int sch  = (code == 10 || code == 11) ? 4 : 3;
    cvt_gold::gold_bgr_to_gray<T>(dst, src, npx, sch, bidx);
    return;
  }
  // Family B: GRAY->BGR/BGRA (codes 8,9)
  if (code == 8 || code == 9) {
    int dch = (code == 9) ? 4 : 3;
    cvt_gold::gold_gray_to_bgr<T>(dst, src, npx, dch);
    return;
  }
  // Family C: packed BGR<->YUV (codes 82,83,84,85)
  if (code >= 82 && code <= 85) {
    int bidx = (code == 82 || code == 84) ? 0 : 2;
    if (code <= 83) cvt_gold::gold_bgr_to_yuv<T>(dst, src, npx, bidx);
    else            cvt_gold::gold_yuv_to_bgr<T>(dst, src, npx, bidx);
    return;
  }
  // Family D: BGR->HSV (codes 40,41,66,67)
  if (code == 40 || code == 41 || code == 66 || code == 67) {
    bool isFullRange = (code == 66 || code == 67);
    int bidx = (code == 40 || code == 66) ? 0 : 2;
    if (isFullRange) cvt_gold::gold_bgr_to_hsv<T, true>(dst, src, npx, bidx);
    else             cvt_gold::gold_bgr_to_hsv<T, false>(dst, src, npx, bidx);
    return;
  }
  // Family D: HSV->BGR (codes 54,55,70,71)
  if (code == 54 || code == 55 || code == 70 || code == 71) {
    bool isFullRange = (code == 70 || code == 71);
    int bidx = (code == 54 || code == 70) ? 0 : 2;
    if (isFullRange) cvt_gold::gold_hsv_to_bgr<T, true>(dst, src, npx, bidx, 3);
    else             cvt_gold::gold_hsv_to_bgr<T, false>(dst, src, npx, bidx, 3);
    return;
  }
  // Family E: YUV2GRAY_420 (code 106)
  if (code == 106) { cvt_gold::gold_yuv420_to_gray<T>(dst, src, W, H, N); return; }
  // Family E: YUV420->BGR (codes 90-105)
  if (code >= 90 && code <= 105) {
    bool isSemi = (code <= 97);
    int bidx = (code % 2 == 0) ? 2 : 0;
    int uidx = isSemi ? ((code % 4 >= 2) ? 0 : 1) : ((code % 4 >= 2) ? 1 : 0);
    if (isSemi) cvt_gold::gold_yuv420_to_bgr_nv12<T>(dst, src, W, H, N, 3, bidx, uidx);
    else        cvt_gold::gold_yuv420_to_bgr_planar<T>(dst, src, W, H, N, 3, bidx, uidx);
    return;
  }
  // Family E: BGR->YUV420 (codes 127-134, 140-147)
  if ((code >= 127 && code <= 134) || (code >= 140 && code <= 147)) {
    ...
    if (isSemi) cvt_gold::gold_bgr_to_yuv420_nv12<T>(dst, src, W, H, N, 3, bidx, uidx);
    else        cvt_gold::gold_bgr_to_yuv420_planar<T>(dst, src, W, H, N, 3, bidx, uidx);
    return;
  }
  // Family F: YUV2GRAY_422 (codes 123,124)
  if (code == 123 || code == 124) {
    int yidx = (code == 123) ? 1 : 0;
    cvt_gold::gold_yuv422_to_gray<T>(dst, src, W, H, N, yidx);
    return;
  }
  // Family F: YUV422->BGR (codes 107-122, excl 109,110,113,114)
  if (code >= 107 && code <= 122) {
    int bidx = (code % 2 == 1) ? 2 : 0;
    int yidx = (code == 107 || code == 108 || code == 111 || code == 112) ? 1 : 0;
    int uidx = (code == 117 || code == 118 || code == 121 || code == 122) ? 2 : 0;
    cvt_gold::gold_yuv422_to_bgr<T>(dst, src, W, H, N, 3, bidx, yidx, uidx);
    return;
  }
  // Unreachable for the profile's code set; no-op to be safe.
}

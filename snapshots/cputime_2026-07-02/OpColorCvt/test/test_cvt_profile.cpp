...
int main() {
  ...
  for (auto& c : cases) {
    ...
    printf("%-22s %10.4f %10.4f %12.1f", c.name, k_ms, best_wall, mpix);

    // Optional CPU gold reference timing (single-thread, 1 run). The profile
    // feeds the SAME host_in bytes to gold (reinterpreted as float for F32) so
    // gold does the same per-pixel work as the GPU kernel.
    const char* ce = std::getenv("VOX_CPUTIME");
    if (ce && ce[0] == '1') {
      auto t0 = std::chrono::steady_clock::now();
      if (c.dtype == DType::F32) {
        std::vector<float> cpu(out_elems);
        cvt_gold_dispatch<float>(cpu.data(), reinterpret_cast<const float*>(host_in.data()),
                                 c.code, N, H, W);
      } else {
        std::vector<uint8_t> cpu(out_elems);
        cvt_gold_dispatch<uint8_t>(cpu.data(), host_in.data(), c.code, N, H, W);
      }
      auto t1 = std::chrono::steady_clock::now();
      double cpu_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
      printf("  cpu=%8.3fms speedup=%7.1fx", cpu_ms, cpu_ms / k_ms);
    }
    printf("\n");
    sycl::free(d_in, q);
    sycl::free(d_out, q);

...
static double bench(sycl::queue& q, int N, int H, int W,
                    ...
                    double& out_gpix, double& out_gbs, double* out_cpu_ms = nullptr) {
    ...
    // Optional CPU gold reference timing (single-thread, 1 run). Built from the
    // same host hsrc/hmap via host-pointer TensorViews. CUBIC on 1080p is slow
    // (~seconds), hence 1 run, not best-of-N.
    if (out_cpu_ms) {
        cs::TensorView<T, C> hsv{hsrc.data(), N, H, W};
        cs::TensorView<T, C> hdv{nullptr, N, H, W};
        cs::TensorView<float, 2> hmv{hmap.data(), 1, H, W};
        std::vector<T> refdst((size_t)N * H * W * C);
        auto t0 = std::chrono::steady_clock::now();
        remap_gold::cpu_remap<T, C>(hsv, hdv, hmv, si, mi, cs::MapValueType::Absolute,
                                    false, b, bv, 1, refdst);
        auto t1 = std::chrono::steady_clock::now();
        *out_cpu_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
    }
...
static void run(sycl::queue& q, const char* tag, int N, int H, int W,
    ...
    double gpix = 0, gbs = 0, cpu_ms = 0;
    double ms = bench<T, C>(q, N, H, W, si, mi, b, iters, gpix, gbs,
                            want_cputime ? &cpu_ms : nullptr);
...
int main() {
    ...
    if (!(ce && ce[0] == '1'))
        std::printf("\n(set VOX_CPUTIME=1 for CPU-vs-GPU speedup per config)\n");
    return 0;
}
